...
def get_network(name, batch_size):
    ...
    keras_model.load_weights(name)
    mod, params = relay.frontend.from_keras(keras_model, {'input_1': input_shape})
...
target_host = tvm.target.create('llvm -target=aarch64-linux-android')
...
def tune_and_evaluate(tuning_opt):
    ...
    print("Extract tasks...")
    mod, params, input_shape, _ = get_network(model_path, batch_size=1)
    tasks = autotvm.task.extract_from_program(mod["main"], target=target, target_host=target_host, params=params,
    ...
    with autotvm.apply_history_best(log_file):
        ...
        data = keras.applications.resnet50.preprocess_input(data).transpose([0, 3, 1, 2])
